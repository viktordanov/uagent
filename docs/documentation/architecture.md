...
- Wrap errors with `fmt.Errorf("failed to <action>: %w", err)`. Add a sentinel only when a caller branches on it: `harness.ErrPreflightBlocked` and `harness.ErrSessionBusy` today. Findings and run statuses are results, not errors.
